...
def load_all_labels(labels_dir: Path) -> pd.DataFrame | None:
    ...
    for f in files:
        ...
        # normalize schema best-effort
                # --- normalize schema best-effort ---
        # Map Customer_ID -> customer_id for joining with scored outputs
...
def compute_auc_if_possible(df_scored: pd.DataFrame, df_labels: pd.DataFrame | None, id_cols: list[str]) -> dict[str, float | None]:
    ...
    # --- normalize columns to lowercase on both sides ---
    df_scored = df_scored.copy()
    df_scored.columns = [c.lower() for c in df_scored.columns]
    ...
    # coerce date type if present
    if "snapshot_date" in df_scored.columns:
        df_scored["snapshot_date"] = pd.to_datetime(df_scored["snapshot_date"], errors="coerce")
    if "snapshot_date" in df_labels.columns:
        df_labels["snapshot_date"] = pd.to_datetime(df_labels["snapshot_date"], errors="coerce")

    right = df_labels[join_on + ["label"]].drop_duplicates()
    merged = df_scored.merge(right, on=join_on, how="left")

    # if nothing matched, skip AUC
    if merged.empty or merged["label"].isna().all():
        return {"lr": None, "xgb": None}

    out = {"lr": None, "xgb": None}
    for model in ["lr", "xgb"]:
        proba_col = f"proba_{model}"
        if proba_col in merged.columns:
             ...
             yb = y_true[valid].astype(float).round().astype(int)
             yp = y_prob[valid].astype(float)
             # Filter to binary subset
             yb = yb[yb.isin([0, 1])]
             yp = yp.loc[yb.index]
             if len(yb) >= 10 and yb.nunique() == 2:
                try:
                    out[model] = float(roc_auc_score(yb, yp))
                except Exception as e:
                    print(f"⚠️  Skipped AUC for {model}: {e}")
                    out[model] = None
    return out
